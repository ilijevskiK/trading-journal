...
// Company "glimpse" data (profile + basic financials + earnings, bundled
// together since all three are always fetched as one unit) — a much longer
// TTL than candles/quotes, since this changes quarterly/daily, not
// intraday.
const FUNDAMENTALS_CACHE_KEY = "tj_fundamentals_cache_v1";
const FUNDAMENTALS_TTL_MS = 24 * 60 * 60 * 1000;

function readFundamentalsCache() {
  try {
...
function writeFundamentalsCache(cache) {
  writeWithEviction(FUNDAMENTALS_CACHE_KEY, cache, "fundamentals cache");
}
...
export function getCachedFundamentals(ticker) {
  ...
  const entry = cache[ticker];
  if (!entry) return null;
  if (Date.now() - entry.fetchedAt > FUNDAMENTALS_TTL_MS) return null;
  return {
...
export function setCachedFundamentals(ticker, { profile, metrics, earnings, peers, news, recommendation }) {
  const cache = readFundamentalsCache();
  cache[ticker] = { profile, metrics, earnings, peers, news, recommendation, fetchedAt: Date.now() };
  writeFundamentalsCache(cache);
}
