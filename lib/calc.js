...
// Calendar days the position was actually held — entry date to its last
// exit date once fully closed, or to today while still open (including
// partially closed). Both dates are plain "YYYY-MM-DD" strings parsed as
// UTC midnight the same way, so the diff is accurate regardless of the
// caller's own timezone.
export function holdDays(trade) {
  if (!trade.entryDate) return null;
  const endDate = isFullyClosed(trade) ? lastExitDate(trade) : todayLocalDateStr();
  if (!endDate) return null;
  const diffMs = new Date(endDate).getTime() - new Date(trade.entryDate).getTime();
  return Math.round(diffMs / (1000 * 60 * 60 * 24));
}

// Average hold time (calendar days) for closed winners vs losers — a
// direct check for the disposition effect (holding losers longer, hoping
// they recover, while cutting winners short) rather than a feeling.
export function holdTimeByOutcome(trades) {
  const closed = trades.filter((t) => isFullyClosed(t));
  const winners = closed.filter((t) => realizedPnl(t) > 0);
  const losers = closed.filter((t) => realizedPnl(t) < 0);
  const avg = (list) =>
    list.length === 0
      ? null
      : round(list.reduce((sum, t) => sum + holdDays(t), 0) / list.length, 1);
  return {
    winners: { count: winners.length, avgDays: avg(winners) },
    losers: { count: losers.length, avgDays: avg(losers) },
  };
}
